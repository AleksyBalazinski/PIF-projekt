...
\begin{enumerate}
    ...
    \item $p$ -- procent pensji, jaki jest odkładany na emeryturę w każdym miesiącu (funkcja pozostałych parametrów).
\end{enumerate}
...
Repozytorium z napisanym kodem, który został wykorzystany do przeprowadzenia omówionych w raporcie obliczeń, dostępne jest pod adresem \url{https://github.com/AleksyBalazinski/PIF-projekt/}.
...
Jeśli przez $S_0$ oznaczymy początkową pensję, to pensja w miesiącu $k$ wynosić będzie $S_0 (1+r_I)^{k - 1}$.
...
Celem weryfikacji powyższego wyprowadzenia, w programie zaimplementowano również funkcję numerycznego wyznaczenia wartości $p$ na podstawie symulacji.
...
\begin{figure}[htbp]
    ...
    \caption{Kapitał końcowy: porównanie wyników uzyskanych metodą symulacji i ze wzoru (\ref{eq:p-exact-formula})}
...
Okazuje się, że jeśli $r_I > r_c$, to zależność $p$ od $r_I$ jest w dobrym przybliżeniu liniowa.
...
\end{equation*}
Wyjaśnia to dlaczego krzywa na \autoref{fig:salary_growth} wydaje się przechodzić w prostą dla duych wartości $r_I$.
...
Celem zbadania interakcji między wybranymi parametrami, narysowano zależność $p$ od par tych parametrów, co przedstawiono na \autoref{fig:main_interaction_plots}.
\begin{figure}[htbp]
    \centering
    % --- Row 1 ---
    \begin{subfigure}[b]{0.48\linewidth}
...
Następnie, celem wyznaczenia $p_u$ podzielono liczbę lat z dodatnią realną stopą zwrotu przez całkowitą rozważaną liczbę lat (za punkt startowy wzięto rok 1980).
...
W tej części projektu przyjęto model opierający się na tzw. strategii \textit{glide path}, tj. strategii której celem jest stopniowa zmiana struktury portfela z bardziej agresywnej (akcje) na bardziej konserwatywną (obligacje) w miarę zbliżania się do daty docelowej $T$.
...
\begin{equation}\label{eq:ws-we-values}
    w_s \in \{0, 0.7, 0.8, 0.9, 1\}, \quad w_e \in \{0, 0.2, 0.3, 0.4, 0.5\}.
\end{equation}
Dla każdej pary $(w_s, w_e)$ wyznaczono optymalną wartość $p$ (tj. najmniejszą, która prowadzi do sukcesu w 95\% przypadków; liczba rozważanych ścieżek to zawsze $10,000$).

Dla rozważanych wartości parametrów, optymalną okazała się strategia z $w_s = 1$, $w_e = 0.5$ i $p=44.64\%$.
Jak zatem widzimy, dodanie do portfela akcji pozwoliło na znaczne zredukowanie procentu $p$ pensji odkładanego co miesiąc na emeryturę.
...
W szczególności zauważyć można długi prawy ogon, który odpowiada rzadkim sytuacjom, w których akcje przynoszą ogromne zyski.
...
\begin{itemize}
    \item Wczesne rozpoczęcie odkładania na emeryturę ($t_0$) radykalnie zmniejsza wymaganą miesięczną stopę oszczędności ($p$). Każdy rok zwłoki skutkuje drastycznym, nieliniowym wzrostem obciążenia bieżącej pensji w późniejszych latach.
    \item Wysoka stopa wzrostu pensji ($I$) -- wbrew intuicji -- zmusza do odkładania większego odsetka dochodów ($p$).
    ...
    \item Model wykazuje znaczą wrażliwość na realną stopę zwrotu z obligacji ($c$) oraz inflację ($\Pi$).
    ...
    \item Wprowadzenie do portfela komponentu akcyjnego (indeksu giełdowego) znacząco poprawia efektywność modelu.
    ...
    \item Portfel oparty wyłącznie o obligacje cechuje się zero-jedynkową funkcją sukcesu.
...
W ramach dalszego rozwoju projektu, naturalnym krokiem byłoby odejście od modeli deterministycznych w sekcji obligacji na rzecz symulacji stochastycznych (np. proces Vasiceka dla stóp procentowych) oraz modelowanie probabilistyczne czasu przeżycia ($t_d$).
